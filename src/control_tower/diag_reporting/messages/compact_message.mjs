export function buildCompactDiagMessage(ctx) {
  ...
  return [
    ...
    '',
    'PIPELINE HEALTH',
    `routeable=${routeable} pairFetchOkRate=${pairFetchRate} preHot(win) considered/passed/failed=${preHotConsideredWin}/${preHotPassedWin}/${preHotFailedWin} preHot(lifetime)=${preHotConsidered}/${preHotPassed}/${preHotFailed} hot(win) enqueued/consumed=${hotEnqWin}/${hotConsWin} hot(lifetime)=${hotEnq}/${hotCons} momentumEvaluated=${cumulativeMomentumEvaluated} confirmReached=${cumulativeConfirmReached} attemptReached=${cumulativeAttempt} fill=${cumulativeFill}`,
    '',
    'PRIMARY CHOKE',
