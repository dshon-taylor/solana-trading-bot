export function buildMomentumDiagMessage(ctx) {
  ...
  return [
    ...
    `- freshnessUnknownBreakdown=missingTelemetryOrLegacy:${Number(freshnessUnknownReasonCounts?.missingTelemetry || 0)} inferredFromWs:${Number(freshnessUnknownReasonCounts?.inferredFromWs || 0)} inferredFromSnapshot:${Number(freshnessUnknownReasonCounts?.inferredFromSnapshot || 0)}`,
    `- medianFreshnessForGateMs=${Number.isFinite(medianFreshnessForGateMs) ? Math.round(medianFreshnessForGateMs) : 'n/a'} medianSnapshotLagOverWsMs=${Number.isFinite(medianSnapshotLagOverWsMs) ? Math.round(medianSnapshotLagOverWsMs) : 'n/a'}`,
    '',
