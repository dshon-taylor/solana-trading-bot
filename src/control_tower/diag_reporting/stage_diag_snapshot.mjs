import { formatThroughputSummary } from '../../observability/metrics.mjs';
...
export function createRefreshDiagSnapshot({ state, getCounters, cfg, birdseye, runtime }) {
  return function refreshDiagSnapshot(nowMs = Date.now()) {
    ...
    const body = [
      ...
      `• momentum.microMetricSource.buySellRatio=${Object.entries(counters?.watchlist?.momentumMicroMetricSourceUsed?.buySellRatio || {}).sort((a,b)=>Number(b[1]||0)-Number(a[1]||0)).slice(0,6).map(([k,v])=>`${k}:${v}`).join(', ') || 'none'}`,
      `• momentum.microMetricSource.tx_1m=${Object.entries(counters?.watchlist?.momentumMicroMetricSourceUsed?.tx_1m || {}).sort((a,b)=>Number(b[1]||0)-Number(a[1]||0)).slice(0,6).map(([k,v])=>`${k}:${v}`).join(', ') || 'none'}`,
      `• momentum.microMetricSource.volume_5m=${Object.entries(counters?.watchlist?.momentumMicroMetricSourceUsed?.volume_5m || {}).sort((a,b)=>Number(b[1]||0)-Number(a[1]||0)).slice(0,6).map(([k,v])=>`${k}:${v}`).join(', ') || 'none'}`,
      `• momentum.neutralFallback.buySellRatio=${Number(counters?.watchlist?.momentumDataNeutralFallback?.buySellRatio || 0)}`,
